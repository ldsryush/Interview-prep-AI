...
### End session + final feedback
`POST /api/interview/session/{sessionId}/end`

---

## Runtime notes
- Frontend and backend are intentionally separate processes.
- Typical local ports:
  - Backend: `8080`
  - Frontend: `3000` (or fallback port if occupied)
- If backend returns errors, frontend displays the backend `message` field directly.
